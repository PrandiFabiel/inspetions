...
function ListVehicleTypes() {
  ...
  function buscar(id) {
    fetch("http://devcompuservi.ddns.net:8080/vehicletype/find?id=" + id)
      .then((res) => res.json())
      .then(
        (result) => {
  ...
  //filtrar
  const [tablaVehicleType, setTablaVehicleType] = useState([]);
  ...
  const filtrar = (terminoBusqueda) => {
    var results = tablaVehicleType.filter((elemento) => {
      if (
        elemento.name
          .toString()
          .toLowerCase()
                ...
                {items.map((item) => (
                  <tr key={item.idVehicleType}>
                    <td
                      ...
                      onClick={() => {
                        buscar(item.idVehicleType);
                        setInputSearch(item.idVehicleType);
                      }}
                    >
                      {item.name}
                    </td>
